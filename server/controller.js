...
module.exports = {
    ...
    getAllClients: (req, res) => {
        sequelize.query(`SELECT * FROM cc_clients a JOIN cc_users c ON a.user_id = c.user_id`)
            .then(dbRes => res.status(200).send(dbRes[0]))
            .catch(err => console.log(err))
    },
    ...
    completeAppointment: (req, res) => {
        sequelize.query(`UPDATE cc_appointments SET completed = true WHERE appt_id = ${req.body.apptId};`)
            .then(dbRes => res.status(200).send(dbRes[0]))
            .catch(err => console.log(err))
    }
}
